...
This is expected when the corresponding tool has not been run yet. Resources reflect state generated by prior tool invocations or direct CLI usage. The MCP tools wrap CLI commands that produce artifacts on disk as a side effect. The resource handlers then read those artifacts. For example:

- `sdp_scout` produces `.sdp/scout.json`, which `sdp://scout` reads.
- `sdp_index_build` produces `.sdp/index.db`, which `sdp://index/modules` and `sdp://index/stats` read.
- `sdp_architect` produces `.sdp/architecture/report.json`, which `sdp://architect` reads.

If a tool has not been run, the resource will not be available. Run the tool first, then query the resource.
