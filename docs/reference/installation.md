...
- **User-scoped:** `.claude/settings.json` (project-local) or `~/.claude/settings.json` (global). Use this for personal preferences.
...
OpenCode uses a `mcp` object (not `mcpServers`) with `"type": "local"` and `"command"` as an array. The `"environment"` field is an object for env vars.
...
This is expected when the corresponding tool has not been run yet. The MCP server does **not** persist artifacts itself -- it delegates to CLI commands, and the CLI decides what to write to disk. Resource handlers then read those artifacts. If a tool has not been run (or the CLI did not produce the expected file), the resource will not be available. For example:

- `sdp_scout` calls `sdp scout`, which produces `.sdp/scout.json`. The `sdp://scout` resource reads that file.
- `sdp_index_build` calls `sdp index build`, which may produce `.sdp/manifest.md`. The `sdp://manifest` resource reads that file.
- `sdp_architect` calls `sdp architect analyze`, which produces `.sdp/architecture/report.json`. The `sdp://architect` resource reads that file.
...
|---------|---------------|----------|
| Claude Code | `.mcp.json` (project) or `.claude/settings.json` (user) | config only |
