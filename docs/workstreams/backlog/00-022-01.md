...
- [ ] `sdp orchestrate --hydrate` writes `.sdp/context-packet.json` before every LLM invocation
- [ ] Packet contains: WS spec, acceptance criteria, scope files list, drift status, checkpoint state, dependency status, quality gate results
- [ ] Each field sourced deterministically (file read, git status, bd show — no LLM)
- [ ] Packet is JSON Schema validated before use
- [ ] LLM prompt includes packet contents (not a file reference — full injection)
- [ ] Hydration failure blocks LLM invocation (fail-safe)
- [ ] Test: packet contents match expected for a sample workstream
