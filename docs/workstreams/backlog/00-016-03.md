...
- [ ] `@oneshot F004` in Claude Code uses `sdp orchestrate` as outer loop
- [ ] Build phases: Task tool spawns subagent for each @build
- [ ] Review phase: Task tool spawns @review subagent
- [ ] PR + CI phases: CLI only, no LLM
- [ ] Stop hook (F015) catches any premature exit attempts
- [ ] No "Next steps" or handoff lists in output
- [ ] Test: complete F001-level feature in Claude Code without premature exit (3/3 runs)
