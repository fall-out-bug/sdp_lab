...
- [x] `provenance.prompt_hash`: SHA-256 of the fully rendered prompt sent to the LLM
- [x] `provenance.context_sources`: array of `{type, path, hash}` — every input that entered the context
- [x] Context source types: `workstream_spec`, `checkpoint`, `scope_file`, `agents_md`, `skill`, `context_packet`
- [x] `sdp-evidence validate` checks `prompt_hash` is a valid SHA-256 hex string
- [x] `sdp-evidence validate` checks `context_sources` is a non-empty array with valid entries
- [x] `sdp-evidence inspect` displays prompt provenance in human-readable format
- [x] Hash computed AFTER all prompt assembly, BEFORE LLM invocation (captures exactly what was sent)
- [x] Backward compatible: envelopes without these fields still validate (fields optional for migration)
- [x] Test: envelope with prompt provenance validates, inspect output includes provenance
