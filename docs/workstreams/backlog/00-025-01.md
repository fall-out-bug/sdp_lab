...
- [ ] All prompt-building logic consolidated into `internal/prompt/` package
- [ ] `TaskSection(ws WorkstreamSpec) string` — renders task description + acceptance criteria
- [ ] `BoundarySection(ws WorkstreamSpec) string` — renders scope files + out-of-scope
- [ ] `EvidenceSection(checkpoint Checkpoint) string` — renders evidence context
- [ ] Each section function is a pure function (no side effects, no file I/O)
- [ ] Golden-file tests for each section (expected output checked in as `.golden` files)
- [ ] Callers (`invoke_opencode.go`, `prompt.go`, `reviewer.go`) refactored to use shared sections
- [ ] Net LOC likely decreases or stays flat
- [ ] No behavioral changes — prompts rendered identically before and after
