...
- [ ] Go build failure → parse error, apply fix, `git commit -m "fix(ci): ..."`, `git push`
- [ ] Go test failure → parse failing test, attempt fix, commit, push
- [ ] k8s-validate failure → parse YAML error, fix manifest, commit, push
- [ ] After fix+push: increment iteration, wait PollDelay (default 60s), re-poll
- [ ] CI_ITER >= max-iter → exit 2 (exceeded)
- [ ] Decision log entry for each fix: `sdp decisions log --decision "AUTO-FIX" --rationale "..."`
- [ ] Test: fix → push → green path; fix → push → still failing → escalate path
