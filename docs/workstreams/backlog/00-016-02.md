...
- [ ] `/oneshot F004` in Cursor calls `sdp orchestrate F004` as the outer loop
- [ ] When outer loop requests "@build 00-004-01", agent executes inline
- [ ] When outer loop requests "@review F004", agent executes inline
- [ ] PR creation and CI loop handled entirely by CLI — no agent involvement
- [ ] Oneshot skill reduced from 8 CRITICAL RULES to 3, positive framing
- [ ] No "Next steps" or handoff lists in output
- [ ] Test: complete F001-level feature in Cursor without premature exit (3/3 runs)
