...
- [ ] `sdp orchestrate F{XX}` can invoke opencode agents via CLI subprocess (`opencode run --agent orchestrator`)
- [ ] `opencode.json` has a documented pattern for routing feature commands to `sdp orchestrate` as outer loop
- [ ] opencode `oneshot` command references the outer loop entrypoint (`sdp orchestrate`) rather than inline workflow
- [ ] opencode lacks native Stop hooks — this WS documents the approved alternative: outer loop CLI replaces Stop hook for opencode
- [ ] Test: `sdp orchestrate F004 --runtime opencode` drives full flow using opencode as inner loop
