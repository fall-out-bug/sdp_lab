...
- [ ] `sdp guard --ws 00-XXX-YY` compares `git diff --name-only` against WS `scope_files`
- [ ] Files in scope → PASS
- [ ] Files outside scope but in allowlist → PASS with warning
- [ ] Files outside scope and not in allowlist → FAIL with list of violating files
- [ ] Allowlist configurable via `.sdp/guard-allowlist.yaml`
- [ ] Default allowlist: `go.sum`, `go.mod`, `package-lock.json`, `yarn.lock`
- [ ] Exit codes: 0 = clean, 1 = out-of-scope changes detected
- [ ] Test: in-scope only, in-scope + allowlist, out-of-scope violation
