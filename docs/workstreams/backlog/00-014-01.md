...
- [ ] `sdp ci-loop --pr 42 --feature F067 --max-iter 5` polls until all checks pass
- [ ] PENDING checks → wait 60s, retry (not counted as iteration)
- [ ] FAILURE checks → classify as auto-fixable or escalate
- [ ] On escalate: `bd create --title="CI BLOCKED: ..." --priority=0`, exit 1
- [ ] On all green: append run event (phase=ci, state=ok) to `.sdp/runs/` run file; checkpoint `phase` updated via SaveCheckpoint; exit 0
- [ ] Reads `.sdp/checkpoints/F{NNN}.json` for pr_number and branch
- [ ] Appends events to `.sdp/runs/` run file
- [ ] Test: poll → green path, poll → failure → escalate path
