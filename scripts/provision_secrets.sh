...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Provisions sdp-credentials secret (github_token, z_ai_api_key, openrouter_api_key)"
...
# Key source: env > opencode.json > fail
...
ssh -p "${PORT}" "${HOST}" "Z_AI_API_KEY_B64='${Z_AI_API_KEY_B64}' OPENROUTER_API_KEY_B64='${OPENROUTER_API_KEY_B64}' NAMESPACES='${NAMESPACES}' bash -s" <<'EOF'
set -euo pipefail
