...
remote_create_issue() {
  ...
  local title_b64 description_b64 spec_b64 parent_b64 workstream_b64
  ...
  ssh -p "${port}" "${host}" "TITLE_B64='${title_b64}' DESC_B64='${description_b64}' SPEC_B64='${spec_b64}' PARENT_B64='${parent_b64}' WORKSTREAM_B64='${workstream_b64}' PRIORITY='${priority}' bash -s" <<'EOF'
set -euo pipefail
...
kubectl -n sdp-workers exec deploy/opencode-agent -- sh -lc "cd /workspace && bd sync --import-only >/dev/null && bd create \"${title}\" -t task -p ${PRIORITY} ${parent_flag} --spec-id \"${spec_id}\" --description \"${description}\" --labels \"autonomy,strict-evidence,model:glm-5,lane:commit,workstream:${workstream}\" --json"
EOF
}
...
create_out="$(remote_create_issue "${HOST}" "${PORT}" "${TITLE}" "${DESCRIPTION}" "${PARENT}" "${PRIORITY}" "${SPEC_ID}" "${WORKSTREAM}")"
