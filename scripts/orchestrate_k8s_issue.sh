...
remote_retry "preflight" "kubectl -n sdp-workers exec deploy/opencode-agent -- sh -lc 'cd /workspace && git rev-parse --is-inside-work-tree >/dev/null && branch=\"\${SDP_REPO_BRANCH:-dev}\" && git fetch origin \"\$branch\" && git rebase FETCH_HEAD && ./scripts/beads_import_only.sh >/dev/null'"
