...
remote_retry "preflight" "kubectl -n sdp-workers exec deploy/opencode-agent -- sh -lc 'cd /workspace && git rev-parse --is-inside-work-tree >/dev/null && bd sync --import-only >/dev/null'"
