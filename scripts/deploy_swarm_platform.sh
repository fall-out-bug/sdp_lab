...
kubectl apply -f "$REPO_ROOT/deploy/k8s/workers/role-agent-coder.yaml"
kubectl apply -f "$REPO_ROOT/deploy/k8s/workers/role-agent-analyst.yaml"

# KEDA scalers (namespace sdp-workers, requires KEDA operator; optional if KEDA not installed)
kubectl apply -f "$REPO_ROOT/deploy/k8s/control/keda-scalers.yaml" 2>/dev/null || echo "[deploy] warning: KEDA scalers skipped (install KEDA operator if needed)"
