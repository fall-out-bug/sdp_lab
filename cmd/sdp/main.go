...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  sdp orchestrate loop [--cycles N] [--interval DURATION]")
	fmt.Fprintln(os.Stderr)
	...
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Architect commands:")
	fmt.Fprintln(os.Stderr, "  sdp architect analyze <repo-path> [--tier 1|2|3] [--format json|text|mermaid] [--output FILE]")
	fmt.Fprintln(os.Stderr, "  sdp architect c4 <repo-path> [--level 1|2|3] [--output DIR]")
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Index commands:")
	fmt.Fprintln(os.Stderr, "  sdp index build [--format json|text] <repo-path>")
	fmt.Fprintln(os.Stderr, "  sdp index refresh [--format json|text] <repo-path>")
	fmt.Fprintln(os.Stderr, "  sdp index stats [--db PATH] <repo-path>")
	fmt.Fprintln(os.Stderr, "  sdp index manifest [--output DIR] [--db PATH] <repo-path>")
	fmt.Fprintln(os.Stderr, "  sdp index query [--format json|text] [--limit N] [--db PATH] <repo-path> <query>")
	fmt.Fprintln(os.Stderr, "  sdp index deps [--reverse] [--depth N] [--db PATH] <repo-path> <module>")
	fmt.Fprintln(os.Stderr, "  sdp index find [--regex] [--limit N] [--db PATH] <repo-path> <term>")
	fmt.Fprintln(os.Stderr, "  sdp index rank [--db PATH] <repo-path>")
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Bootstrap commands:")
	fmt.Fprintln(os.Stderr, "  sdp bootstrap [--dry-run] [--force] [--beads] [--yes] [--auto-curate] [--format json|text] [--only TYPES] [--conventions] [--mode greenfield|brownfield] [--preset NAME] [--no-verify] <repo-path>")
	...
	fmt.Fprintln(os.Stderr, "  sdp generate-adapters [--manifest <path>] [--out <dir>] [--check|--write|--diff]")
	fmt.Fprintln(os.Stderr)
	...
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Tower commands:")
	fmt.Fprintln(os.Stderr, "  sdp tower [--addr <host:port>]   Start local control tower UI/server")
}
