...
func TestCoverageFormatJSON(t *testing.T) {
	...
	if len(parsed) != 2 {
		t.Fatalf("expected 2 items in JSON, got %d", len(parsed))
	}
	if parsed[0].File != "a.go" {
		t.Errorf("parsed[0].File = %q, want %q", parsed[0].File, "a.go")
	}
	if parsed[0].Coverage != 50.0 {
		t.Errorf("parsed[0].Coverage = %.1f, want 50.0", parsed[0].Coverage)
	}
	if parsed[0].ThresholdGap != 30.0 {
		t.Errorf("parsed[0].ThresholdGap = %.1f, want 30.0", parsed[0].ThresholdGap)
	}
}
...
func TestCoverageFormatJSONEmpty(t *testing.T) {
	...
	if !strings.Contains(out, "[]") {
		t.Errorf("empty gaps JSON should be [], got: %s", out)
	}
}
