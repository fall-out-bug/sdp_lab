...
const (
	draftHeaderPrefix = "<!-- DRAFT: generated by sdp bootstrap "
	draftTODOComment  = "<!-- TODO: verify this rule matches intended behavior -->"
)
...
// StripDraftDecorations removes the DRAFT header and review TODO markers so a
// draft artifact can be safely re-merged and re-wrapped without duplication.
func StripDraftDecorations(content string) string {
	if strings.HasPrefix(content, draftHeaderPrefix) {
	...
	content = strings.ReplaceAll(content, draftTODOComment+"\n", "")
	return strings.ReplaceAll(content, draftTODOComment, "")
}
