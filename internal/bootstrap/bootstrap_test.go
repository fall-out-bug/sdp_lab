...
func TestPlanner_Status_WithDraftFiles(t *testing.T) {
	dir := setupRepoWithScout(t)
	require.NoError(t, os.WriteFile(filepath.Join(dir, "DRAFT-CLAUDE.md"), []byte("# claude"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "DRAFT-AGENTS.md"), []byte("# agents"), 0o644))
...
func TestPlanner_Execute_DraftForceMerge_DoesNotDuplicateDraftMarkers(t *testing.T) {
	dir := setupRepoWithScout(t)
	...
	initialContent, err := os.ReadFile(filepath.Join(dir, "DRAFT-CLAUDE.md"))
	require.NoError(t, err)

	forcePlanner := NewPlanner(BootstrapConfig{
		RepoPath: dir,
		UseDraft: true,
		Force:    true,
		NoVerify: true,
	})
	_, err = forcePlanner.Execute()
	require.NoError(t, err)

	mergedContent, err := os.ReadFile(filepath.Join(dir, "DRAFT-CLAUDE.md"))
	require.NoError(t, err)

	assert.Equal(t, 1, strings.Count(string(initialContent), "<!-- DRAFT:"))
	assert.Equal(t, strings.Count(string(initialContent), "<!-- DRAFT:"), strings.Count(string(mergedContent), "<!-- DRAFT:"))
	assert.Equal(t,
		strings.Count(string(initialContent), "<!-- TODO: verify this rule matches intended behavior -->"),
		strings.Count(string(mergedContent), "<!-- TODO: verify this rule matches intended behavior -->"))
}
