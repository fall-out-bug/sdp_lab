...
const validPICorpusSchemaJSON = `{
  ...
  "properties": {
    ...
    "allowed_tools": {
      "type": "array",
      "items": { "type": "string" },
      "description": "Allowlist of tools permitted for this case"
    },
    "forbidden_tools": {
      "type": "array",
      "items": { "type": "string" },
...
func (v *PICorpusValidator) ValidateAll() []ValidateResult {
	...
	if err := json.Unmarshal(schemaData, &schema); err != nil {
		return []ValidateResult{{Case: "schema", Pass: false, Reason: fmt.Sprintf("invalid schema JSON: %v", err)}}
	}
	...
	for _, p := range matches {
		...
		caseResults = append(caseResults, v.validateCase(c, schema))
	}
...
func (v *PICorpusValidator) validateCase(c PICase, schema map[string]any) ValidateResult {
	var failures []string
	...
	if len(c.AllowedTools) == 0 {
		failures = append(failures, "allowed_tools must have at least one entry (or empty array for no tools)")
	}
	if len(c.ExpectedBehavior) == 0 {
		failures = append(failures, "expected_behavior must have at least one entry")
	}

	// Class validation: keyword_block_only is not a valid attack class (it's a failure fixture)
	validClasses := map[string]bool{
		...
		"benign_control":                 true,
		// keyword_block_only is intentionally excluded — it fails validation as a fixture
	}
	if c.Class != "" && !validClasses[c.Class] {
		failures = append(failures, fmt.Sprintf("invalid class %q (keyword_block_only is not a valid attack class — use proper attack classes)", c.Class))
	}

	// Authoritative layer validation
